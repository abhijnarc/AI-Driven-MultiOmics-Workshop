"""Insert new AE-comparison cells into notebooks/01_disease_subtyping_vae.ipynb
after the evaluation section (after cell id 5f05f204), embedding captured outputs."""
import json, io, uuid

NB_PATH = "notebooks/01_disease_subtyping_vae.ipynb"
RESULTS = "_cell_results.json"
...
# ── Locate insertion point: after the cluster-composition cell (id 5f05f204) ──
insert_idx = None
for i, c in enumerate(nb["cells"]):
    if c.get("id") == "5f05f204":
        insert_idx = i + 1
        break
assert insert_idx is not None, "Anchor cell 5f05f204 not found"

def md(source):
    return {"cell_type": "markdown", "id": uuid.uuid4().hex[:8],
            "metadata": {}, "source": source}

def code(source, outputs_text, image_files=None):
...
CODE_SETUP = '''import random, time, json
...
CODE_COMPARE = '''# ── Rebuild the trained VAE exactly as in Part 15 and evaluate both on val set ─
...
CODE_UMAP = '''import umap as umap_lib
...
new_cells = [
    md(MD_INTRO),
    code(CODE_SETUP, results["setup"]["outputs"]),
    code(CODE_AE, results["ae_train"]["outputs"]),
    md(MD_BETWEEN),
    code(CODE_COMPARE, results["compare"]["outputs"]),
]

if "umap" in results:
    new_cells.append(md(MD_UMAP))
    new_cells.append(code(CODE_UMAP, results["umap"]["outputs"]))
...
print(f"Inserted {len(new_cells)} cells at position {insert_idx} into {NB_PATH}")